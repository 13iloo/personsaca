...
export default class MSSequelizeStorage {
  constructor({
    ...
    if (dialectOptions) this.dialectOptions = dialectOptions;
    else
      this.dialectOptions = {
        options: {
          instanceName: "",
  ...
  static async ConnectCreateAndSeed(app) {
    ...
    try {
      app.sequelizeStorage = new MSSequelizeStorage({
        ...
        database: process.env.DB_NAME || "ElementDB",
        username: process.env.DB_USERNAME || "sa",
        password: process.env.DB_PASSWORD || "Qaz@wsx!",
        dialect: "mssql",
        domain: process.env.DB_DOMAIN || null,
        // dialectOptions: {
        //     options: {
        //         instanceName: 'SQLEXPRESS', // <= choose the right one
        //         encrypt: true,
        //         trustServerCertificate: true, // <= for self issued certificate
        //     }
        // }
  ...
  async connect({
    ...
    try {
      ...
      if (dialect) this.dialect = dialect;
      if (dialectOptions) this.dialectOptions = dialectOptions;
      ...
      if (this.domain)
        ...
        await this.sequelize.query(
          `IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = N'${this.database}') CREATE DATABASE [${this.database}];`
        );
      ...
      // Test connection
      const connectionResult = await this.sequelize.authenticate();
